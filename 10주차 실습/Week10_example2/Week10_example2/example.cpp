...
void Example::init()
{
	...
	blendShader->make_shader("blend.vs", "blend.fs");
...
void Example::draw()
{
	...
	// Cube
	blendShader->use_program();
	blendShader->setMat4("view", camera->getViewMat());
	blendShader->setMat4("proj", camera->getProjMat());

	glEnable(GL_BLEND);
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	for (int i = 0; i < 6; i++)
	{
		cube->setWorldMat(1.0f);
		if (i < 3)
			cube->setWorldTranslate(-1.0f, 0.2f, 1.0f - 1.0f * (i % 3));
		else
			cube->setWorldTranslate(1.0f, 0.2f, 1.0f - 1.0f * (i % 3));
		cube->setWorldScale(1.0f, 2.0f, 1.0f);
